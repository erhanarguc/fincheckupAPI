...
public sealed class WzonerSWRService : IWzonerSWRService
{
    ...
    private const string CacheKey = "Wzoner:all";
    ...
    {
        public IReadOnlyList<TBLErrzoneRow> Data { get; init; } = Array.Empty<TBLErrzoneRow>();
    ...
    public WzonerSWRService(IMemoryCache cache, IConfiguration cfg)
    {
        _cache = cache;
    ...
    public async Task<IReadOnlyList<TBLErrzoneRow>> GetAllAsync(CancellationToken ct = default)
    {
    ...
    private async Task<List<TBLErrzoneRow>> ReadRows(CancellationToken ct)
    {
        using var con = new SqlConnection(_connStr);
        var list = (await con.QueryAsync<TBLErrzoneRow>(
            "SELECT [ID],[MainDescription],[ColorDesc],[Description],[ColorDescTax],[DescriptionTax],[ColorDescInside],[DescriptionInside],[CreatedDate]  FROM  [dbo].[TBLErrzone] WITH (NOLOCK)")).ToList();

        return list;
    }

    private static Container BuildContainer(List<TBLErrzoneRow> rows, DateTimeOffset now) => new()
    {
        Data = rows,
        LoadedAt = now,
