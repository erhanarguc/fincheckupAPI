...
public sealed class WChkSWRService: IWChkSWRService
{
    ...
    private const string CacheKey = "WChk:all";
    ...
    {
        public IReadOnlyList<ErrorCheckSet> Data { get; init; } = Array.Empty<ErrorCheckSet>();
    ...
    public WChkSWRService(IMemoryCache cache, IConfiguration cfg)
    {
        _cache = cache;
    ...
    public async Task<IReadOnlyList<ErrorCheckSet>> GetAllAsync(CancellationToken ct = default)
    {
    ...
    private async Task<List<ErrorCheckSet>> ReadRows(CancellationToken ct)
    {
        using var con = new SqlConnection(_connStr);
        var list = (await con.QueryAsync<ErrorCheckSet>(
            "SELECT [ID],[MainDescription],[DebitCreditCode],[ColorDesc],[Description],[ColorDescTax],[DescriptionTax],[ColorDescInside] ,[DescriptionInside] ,[InMainDesc],[InMainDescDC],[OutMainDesc],[OutMainDescDC],[CheckAmount],[QueryType] FROM dbo.[TBLErrzoneInside] WITH (NOLOCK)")).ToList();

        return list;
    }

    private static Container BuildContainer(List<ErrorCheckSet> rows, DateTimeOffset now) => new()
    {
        Data = rows,
        LoadedAt = now,
